...
router.get('/house',(req,res)=>{
	...
	req.db.query(`SELECT ID,title,ave_pice,tel FROM house_table LIMIT ${start},${size}`,(err,house_data)=>{
		if(err){
			res.sendStatus(500),
			res.end();
		}else{
			...
			req.db.query('SELECT COUNT(*) AS c FROM house_table',(err,data)=>{
				if(err){
					...
					res.render('index',{data:house_data,page_count:Math.ceil(data[0].c/size),cur_page:page});	//向上取整ceil
				}
			})
		}
	})
...
router.post('/house',(req,res)=>{
	...
	//时间
	let sale_time=Math.floor(new Date(req.body['sale_time']).getTime()/1000);
	let submit_time=Math.floor(new Date(req.body['submit_time']).getTime()/1000);
